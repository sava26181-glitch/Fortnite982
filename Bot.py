...
def cmd_start(message):
    ...
    bot.send_message(
        ...
        "Нажми на кнопку ниже, чтобы открыть полноценное приложение с чёрно-синим интерфейсом.",
        parse_mode="HTML",
        reply_markup=markup
    )
...
def handle_web_app_data(message):
    try:
        ...
        if command == 'bruteforce':
            ...
            bot.reply_to(message, "⏹ Остановка пока не реализована.")
        else:
...
def run_auto_mass(message, count):
    bot.reply_to(message, f"🚀 Генерация {count} email и проверка...\n⏳ Процесс запущен в фоне.")
