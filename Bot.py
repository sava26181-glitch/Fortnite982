...
# ============================================================
#  ОФОРМЛЕНИЕ И КНОПКИ (ЧЁРНО-СИНИЙ СТИЛЬ)
# ============================================================
...
def cmd_start(message):
    ...
    btn1 = types.InlineKeyboardButton("🔍 Брутфорс (ввести email)", callback_data="bruteforce")
    ...
    btn4 = types.InlineKeyboardButton("📊 Статус", callback_data="status")
...
def handle_callback(call):
    ...
    if call.data == "bruteforce":
        bot.send_message(call.message.chat.id, style_message("📧 Введите email командой:\n/bruteforce <email>"), parse_mode="HTML")
    elif call.data == "auto_mass":
        bot.send_message(call.message.chat.id, style_message("⚡ Запускаю массовый подбор (20 000 email)...\n⏳ Результаты придут позже."), parse_mode="HTML")
        threading.Thread(target=run_auto_mass, args=(call.message, 20000)).start()
...
# ============================================================
#  ОСТАЛЬНЫЕ КОМАНДЫ (с поддержкой кнопок)
# ============================================================
