...
BOT_TOKEN = "8874004875:AAEslk0sxxDKXNnWtvggCc3RKUTJB4NwV14"  # вставьте свой токен
bot = telebot.TeleBot(BOT_TOKEN)

# ================== РАСШИРЕННЫЙ СЛОВАРЬ (20 000+) ==================
BASE_WORDS = [
...
def handle_callback(call):
    bot.answer_callback_query(call.id)  # сразу убираем часики
    ...
    if call.data == "bruteforce":
        ...
        bot.send_message(call.message.chat.id, "⚡ Запускаю массовый подбор (20 000 email)...\n⏳ Ожидайте, процесс может занять несколько минут.")
        # Запускаем долгую функцию в отдельном потоке, чтобы не блокировать бота
        threading.Thread(target=cmd_auto_mass, args=(call.message,)).start()
...
# ================== ПОЛУЧЕНИЕ FILE_ID (не влияет на работу) ==================
